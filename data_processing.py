...
def rmsle_loss(pred, y):
    ...
    return torch.sqrt(torch.mean((torch.log1p(pred) - torch.log1p(y)) ** 2))
...
class SalesDataset(Dataset):
    def __init__(self, df, seq_cols, train=True):
        self.seq = torch.tensor(df[seq_cols].values, dtype=torch.float32).view(len(df), -1, len(seq_cols))
...
def load_datasets(data_dir):
    ...
    for l in [7, 14, 28]:
        df[f"lag_{l}"] = df.groupby(["store_nbr", "family"])["sales"].shift(l)

    df = df.dropna().reset_index(drop=True)

    seq_cols = ["lag_7", "lag_14", "lag_28", "onpromotion", "oil", "is_holiday"]

    tr, va = train_test_split(df, test_size=0.2, shuffle=False)
    ...
    return (
        SalesDataset(tr, seq_cols, True),
        SalesDataset(va, seq_cols, True),
        meta,
    )
